...
export const OrderProcessingModal = ({ orders, items, bomItems, onStartProcessing, onShip, onReceive, onClose, currentUserBranch, isAdmin }: OrderProcessingModalProps) => {
  ...
  const getFinishedItemName = (order: Order) => {
    return items.find(item => item.id === order.finishedItemId)?.name || '알 수 없음';
  };
  ...
  const handleToggleOrder = (orderId: string) => {
    ...
    if (newSelected.has(orderId)) {
      ...
      if (order) {
        setShippingQuantities(prev => ({ ...prev, [orderId]: order.quantity }));
      }
    }
  ...
  const handleQuantityChange = (orderId: string, quantity: number) => {
    ...
    setShippingQuantities(prev => ({ ...prev, [orderId]: validQuantity }));
  };
  ...
  const handleShipSelected = (e: FormEvent) => {
    ...
    orderIds.forEach(orderId => {
      ...
      if (!quantity || quantity <= 0) hasError = true;
    });
    ...
    orderIds.forEach(orderId => {
      onShip(orderId, shippingQuantities[orderId]);
    });
  ...
  const branchesToShow = isAdmin ? Object.keys(ordersByBranch) : currentUserBranch ? [currentUserBranch] : [];
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '900px', maxHeight: '90vh', overflow: 'auto' }}>
        ...
        <div className="order-processing-content">
          {branchesToShow.map(branchName => {
            ...
            return (
              <div key={branchName} className="branch-orders-section" style={{ marginBottom: '32px' }}>
                <h3 style={{ marginBottom: '16px', color: '#1a73e8', fontSize: '20px' }}>{branchName}</h3>
                ...
                {pendingOrders.length > 0 && isAdmin && (
                  <div className="order-status-group">
                    ...
                    <div className="order-list-table">
                        ...
                        <tbody>
                          {pendingOrders.map(order => (
                            <tr key={order.id} style={{ borderBottom: '1px solid #e0e0e0' }}>
                              <td style={{ padding: '12px', textAlign: 'center' }}><button type="button" onClick={() => handleToggleOrder(order.id)} className="icon-btn" style={{ border: 'none', background: 'none', cursor: 'pointer' }}>{selectedOrders.has(order.id) ? <CheckSquare size={20} color="#1a73e8" /> : <Square size={20} color="#9aa0a6" />}</button></td>
                      ...
                      {selectedOrders.size > 0 && <div style={{ marginTop: '16px', textAlign: 'right' }}><button type="button" onClick={handleStartProcessing} className="btn btn-primary">처리 시작 ({selectedOrders.size}개)</button></div>}
                    </div>
                  </div>
                )}
                            ...
                            {processingOrders.map(order => {
                              ...
                              return (
                                <tr key={order.id} style={{ borderBottom: '1px solid #e0e0e0' }}>
                                  ...
                                  <td style={{ padding: '12px' }}>{getFinishedItemName(order)}</td>
                                  <td style={{ padding: '12px', textAlign: 'right' }}>{order.quantity}개</td>
                                  <td style={{ padding: '12px', textAlign: 'right' }}><input type="number" min="1" max={order.quantity} value={shippingQty} onChange={(e) => handleQuantityChange(order.id, Number(e.target.value))} style={{ width: '80px', padding: '6px', border: '1px solid #dadce0', borderRadius: '4px' }} required /><span style={{ marginLeft: '4px', color: '#5f6368' }}>/ {order.quantity}개<span style={{ marginLeft: '8px', color: completionRate >= 100 ? '#48bb78' : '#ed8936', fontWeight: 600 }}>({completionRate}%)</span></span></td>
                                  <td style={{ padding: '12px' }}>{getOrderBOM(order).length > 0 ? <div style={{ fontSize: '12px', color: '#5f6368' }}>{getOrderBOM(order).map((bom, idx) => { const material = items.find(item => item.id === bom.materialItemId); const qty = shippingQuantities[order.id] || order.quantity; return <div key={bom.id}>{material?.name}: {bom.quantity * qty} {material?.unit}{idx < getOrderBOM(order).length - 1 && ', '}</div>; })}</div> : <span style={{ color: '#9aa0a6' }}>BOM 미설정</span>}</td>
                                </tr>
                              );
                            })}
                          </tbody>
                        </table>
                        {selectedOrders.size > 0 && <div style={{ marginTop: '16px', textAlign: 'right' }}><button type="submit" className="btn btn-primary"><Truck size={18} />출고 처리 ({selectedOrders.size}개)</button></div>}
                      </div>
                    </form>
                  </div>
                )}
                        ...
                        <tbody>
                          {shippingOrders.map(order => {
                            ...
                            return (
                              <tr key={order.id} style={{ borderBottom: '1px solid #e0e0e0' }}>
                                <td style={{ padding: '12px' }}>{getFinishedItemName(order)}</td>
                                <td style={{ padding: '12px', textAlign: 'right' }}>{shippedQty}개 / {order.quantity}개<span style={{ marginLeft: '8px', color: completionRate >= 100 ? '#48bb78' : '#ed8936', fontWeight: 600 }}>({completionRate}%)</span></td>
                                <td style={{ padding: '12px' }}>{order.shippedAt ? formatDate(order.shippedAt) : '-'}</td>
                                <td style={{ padding: '12px' }}>{!isAdmin && <button type="button" onClick={() => handleReceive(order.id)} className="btn btn-primary btn-small"><PackageCheck size={16} />입고 처리</button>}</td>
                              </tr>
                            );
                          })}
                        </tbody>
                        ...
                        <tbody>
                          {receivedOrders.map(order => (
                            <tr key={order.id} style={{ borderBottom: '1px solid #e0e0e0' }}>
                              ...
                              <td style={{ padding: '12px' }}>{order.receivedAt ? formatDate(order.receivedAt) : '-'}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
          ...
          {branchesToShow.length === 0 && <div className="empty-state" style={{ padding: '40px', textAlign: 'center', color: '#5f6368' }}>처리할 주문이 없습니다.</div>}
        </div>
      </div>
    </div>
  );
