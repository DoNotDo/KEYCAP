...
import { X, Package, Clock, CheckCircle, AlertTriangle, Truck, PackageCheck } from 'lucide-react';
        ...
        <div className="branch-detail-content">
          <div className="branch-stats-section" style={{ marginBottom: '24px', padding: '16px', backgroundColor: '#f8f9fa', borderRadius: '8px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '12px' }}>
              <div style={{ textAlign: 'center', padding: '12px', backgroundColor: '#fff', borderRadius: '4px' }}><div style={{ fontSize: '24px', fontWeight: 700, color: '#1a73e8' }}>{stats.total}</div><div style={{ fontSize: '12px', color: '#5f6368', marginTop: '4px' }}>전체</div></div>
              <div style={{ textAlign: 'center', padding: '12px', backgroundColor: '#fff', borderRadius: '4px' }}><div style={{ fontSize: '24px', fontWeight: 700, color: '#ed8936' }}>{stats.pending}</div><div style={{ fontSize: '12px', color: '#5f6368', marginTop: '4px' }}>대기</div></div>
              <div style={{ textAlign: 'center', padding: '12px', backgroundColor: '#fff', borderRadius: '4px' }}><div style={{ fontSize: '24px', fontWeight: 700, color: '#4299e1' }}>{stats.processing}</div><div style={{ fontSize: '12px', color: '#5f6368', marginTop: '4px' }}>처리중</div></div>
              <div style={{ textAlign: 'center', padding: '12px', backgroundColor: '#fff', borderRadius: '4px' }}><div style={{ fontSize: '24px', fontWeight: 700, color: '#9f7aea' }}>{stats.shipping}</div><div style={{ fontSize: '12px', color: '#5f6368', marginTop: '4px' }}>출고</div></div>
              <div style={{ textAlign: 'center', padding: '12px', backgroundColor: '#fff', borderRadius: '4px' }}><div style={{ fontSize: '24px', fontWeight: 700, color: '#38b2ac' }}>{stats.received}</div><div style={{ fontSize: '12px', color: '#5f6368', marginTop: '4px' }}>입고</div></div>
              <div style={{ textAlign: 'center', padding: '12px', backgroundColor: '#fff', borderRadius: '4px' }}><div style={{ fontSize: '24px', fontWeight: 700, color: '#48bb78' }}>{stats.completed}</div><div style={{ fontSize: '12px', color: '#5f6368', marginTop: '4px' }}>완료</div></div>
            </div>
          </div>
          ...
          {branchShortage && branchShortage.totalShortageCount > 0 && (
            <div className="branch-shortage-section" style={{ marginBottom: '24px', padding: '16px', backgroundColor: '#fff3cd', borderRadius: '8px', border: '1px solid #ffc107' }}>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: '12px' }}><AlertTriangle size={20} color="#ff9800" /><h3 style={{ margin: '0 0 0 8px', color: '#ff9800' }}>부자재 부족 알림</h3></div>
              <div style={{ fontSize: '14px', color: '#5f6368' }}>
                {branchShortage.shortages.map((shortage, idx) => (
                  <div key={idx} style={{ marginBottom: '8px' }}><strong>{shortage.materialName}</strong>: 필요 {shortage.requiredQuantity}개, 현재 {shortage.availableQuantity}개 (부족: {shortage.shortage}개)</div>
                ))}
              </div>
            </div>
          )}
          ...
          <div className="branch-orders-section">
            ...
            {branchOrders.length === 0 ? <p className="empty-state">발주 내역이 없습니다.</p> : (
                  ...
                  return (
                    <div key={order.id} className={`order-card ${order.status}`} style={{ marginBottom: '12px' }}>
                      <div className="order-card-header">
                        <div className="order-info"><div className="order-item-name"><Package size={20} /><strong>{getItemName(order.finishedItemId)}</strong></div></div>
                        <div className="order-status-badge" style={{ color: getStatusColor(order.status) }}>{getStatusIcon(order.status)}<span>{order.status === 'pending' ? '대기' : order.status === 'processing' ? '처리중' : order.status === 'shipping' ? '출고' : order.status === 'received' ? '입고' : order.status === 'completed' ? '완료' : order.status}</span></div>
                      </div>
