...
export const StatsDetailModal = ({ type, stats, items, onClose }: StatsDetailModalProps) => {
  ...
  const getFilteredItems = () => {
    switch (type) {
      ...
      case 'totalValue':
        return items.sort((a, b) => (b.quantity * b.price) - (a.quantity * a.price));
    }
  };
          ...
          {type === 'totalValue' && (
            <div className="value-summary">
              <div className="value-summary-item">
                <span className="label">총 재고 가치:</span>
                <span className="value">{stats.totalValue.toLocaleString()}원</span>
              </div>
            </div>
          )}
            ...
            <table className="stats-detail-table">
              <thead>
                <tr>
                  ...
                  <th>타입</th>
                  <th>재고</th>
                  {type === 'totalValue' && <th>단가</th>}
                  {type === 'totalValue' && <th>총 가치</th>}
                  {type === 'lowStock' && <th>최소 수량</th>}
                  {type === 'lowStock' && <th>부족량</th>}
                </tr>
              </thead>
              <tbody>
                {filteredItems.length === 0 ? (
                  <tr>
                    <td colSpan={type === 'totalValue' ? 5 : type === 'lowStock' ? 5 : 3} className="empty-state">
                      항목이 없습니다.
                    </td>
                  </tr>
                ) : (
                      ...
                      <tr key={item.id} className={type === 'lowStock' ? 'low-stock-row' : ''}>
                        ...
                        {type === 'totalValue' && (
                          <>
                            ...
                            <td className="value-cell">{(item.quantity * item.price).toLocaleString()}원</td>
                          </>
                        )}
                        {type === 'lowStock' && (
                          <>
                            <td>{item.minQuantity} {item.unit}</td>
                            <td className="shortage-cell">{shortage} {item.unit}</td>
                          </>
                        )}
                      </tr>
                    );
